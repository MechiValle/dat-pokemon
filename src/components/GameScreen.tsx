...
import CryChoices from './CryChoices';
...
export default function GameScreen({
  ...
  const remaining = useCountdownTimer(
    QUICK_ROUND_SECONDS,
    mode === 'quick' && isPlaying,
    onTimeout,
  );

  const displaySeconds = mode === 'quick' ? remaining : elapsedUp;
  const elapsedForResults =
    mode === 'quick' ? QUICK_ROUND_SECONDS - remaining : elapsedUp;
  ...
  const isWrong =
    isRevealed &&
    (currentRound.status === 'incorrect' || currentRound.status === 'passed');
  ...
  return (
    <div className='flex flex-col items-center gap-4 py-6 w-full'>
      ...
      >
        ...
        <div className='rounded-xl bg-screen-light-inner dark:bg-screen-dark p-5'>
          {roundSource === 'cries' ? (
            <CryChoices
              pokemon={currentRound.pokemon}
              pool={fullPool}
            ...
            <div className='flex flex-col md:flex-row gap-6 md:items-center'>
              ...
              <div className='flex-1'>
                <AutocompleteInput
                  ...
                  onPass={pass}
                  disabled={isRevealed}
                  showSuggestions={handicapMode === 'handicap'}
                />
              </div>
            </div>
          )}
