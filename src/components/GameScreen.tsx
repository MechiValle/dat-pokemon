...
export default function GameScreen({ pool, onFinish, onBackToMenu }: GameScreenProps) {
  const { t } = useTranslation();
  const { currentRound, correctCount, total, matchStatus, isRevealed, submitGuess, pass } =
    useGame(pool);
  const elapsedSeconds = useTimer(matchStatus === "playing");
  const pokemonNames = pool.map((p) => p.name);

  useEffect(() => {
    if (matchStatus === "finished") {
      onFinish(correctCount, total, elapsedSeconds);
    }
  ...
  const isWrong = isRevealed && (currentRound.status === "incorrect" || currentRound.status === "passed");
